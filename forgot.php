...
$con=mysqli_connect("localhost","root","a","hostel");
...
$result = mysqli_query($con,"SELECT * FROM student WHERE USN='$username' AND email_id='$email'");
if (mysqli_error($con))
{
...
// -----Update pass in DB-----
	echo "<br>";
	$newpass = md5($newpass);
	mysqli_query($con,"UPDATE login SET password='$newpass' WHERE username='$username' ");
	if (mysqli_error($con))
	{
   		die(mysqli_error($con));
	}
